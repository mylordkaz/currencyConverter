...
//   - frontend/src/lib/currency.ts   (this file)
//   - mobile/lib/currency.ts
// Keep the two copies in sync. The logic is intentionally self-contained (no
// framework/platform imports) so it can be copied across unchanged.
//
...
export interface Currency {
  /** Stable unique id: `fiat-<code>` (e.g. `fiat-USD`) or `crypto-<coinpaprika id>` (e.g. `crypto-btc-bitcoin`). */
  id: string;
  code: string;
  flag: string;
  /** Fiat: units per 1 USD. Crypto: price of 1 unit in USD. */
  rate: number;
  name: string;
  symbol: string;
  type: 'fiat' | 'crypto';
